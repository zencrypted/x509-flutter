...
@usableFromInline struct CertificateExtensions_BasicConstraintsSyntax: DERImplicitlyTaggable, Sendable {
...
if let next = peek_pathLenConstraint.next(), next.identifier == ArraySlice<UInt8>.defaultIdentifier {
    pathLenConstraint = try ArraySlice<UInt8>(derEncoded: &nodes)
            ...
            return CertificateExtensions_BasicConstraintsSyntax(cA: cA, pathLenConstraint: pathLenConstraint)
        }
    }
